...
/* On a unicast edge->edge REGISTER (A wants B), advertise R to both parties
 * so they start registering to R and can use it once direct fails. */
static void advertise_relay_on_register( n2n_sn_t *sss,
                                         const n2n_common_t * cmn,
                                         const n2n_mac_t req_mac,
                                         const n2n_mac_t tgt_mac )
{
    struct peer_info * relay = find_community_relay( sss, cmn->community, req_mac );
    if ( !relay ) return; /* no good peer; stay on plain SN relay */

    struct peer_info * req = find_peer_by_mac( sss->edges, req_mac );
    struct peer_info * tgt = find_peer_by_mac( sss->edges, tgt_mac );
    /* Make sure the would-be relay actually wants to serve (it must be a
     * member; R serves whoever registers to it). Also ask R to accept their
     * registrations by advertising both members to R is unnecessary: R learns
     * them when A/B register to it. */
    if ( req ) advertise_relay_to( sss, cmn, req, relay );
                        ...
                        ws_conn_t * ws_sender)
{
    ...
    else if ( msg_type == MSG_TYPE_REGISTER )
    {
        ...
        advertise_relay_on_register( sss, &cmn, reg.srcMac, reg.dstMac );
