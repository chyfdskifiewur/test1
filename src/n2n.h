...
/* Peer-relay eligibility (both sn scoring and edge self-check use this):
 * only NAT1 (full-cone) and NAT2 (cone / address-restr) qualify. A
 * port-restr relay cannot form a direct connection with symmetric endpoints
 * (its NAT filters inbound by exact IP:port, which a symmetric source never
 * matches), and "unknown" is simply unmeasured. */
...
struct peer_info {
    ...
    uint8_t             relay_mode;        /* sn: peer-relay willingness (0=refuse 1=auto 2=willing) */
...
/* Peer-relay assignment (edge side): traffic to dst_mac is sent to the
 * relay edge instead of the supernode while the assignment is valid.
 * The relay peer is picked by the supernode (willing edges first, then
 * auto-consent edges) and refreshed before expiry. */
#define N2N_EDGE_RELAY_MAX  4
...
/* R role: the source address each sender last used to reach us. Forwarding
 * back to that address is the only path symmetric / port-restricted
 * destinations reliably accept (their registered sn-mapping is not open
 * to the relay). */
#define N2N_EDGE_RELAY_RT_MAX   8
typedef struct n2n_relay_rt {
    n2n_mac_t   mac;
    n2n_sock_t  sock;
    time_t      seen;
} n2n_relay_rt_t;
...
struct n2n_edge
{
    ...
    /* Peer-relay: user-configured willingness (-Z) plus assignments the
     * supernode granted (dst_mac -> relay edge). Data plane only; the
     * supernode stays the signalling anchor. */
    uint8_t             relay_mode;     /* 0=refuse 1=auto(default) 2=willing */
    n2n_relay_entry_t   relay_table[N2N_EDGE_RELAY_MAX];
    n2n_relay_rt_t      relay_rt[N2N_EDGE_RELAY_RT_MAX]; /* R: sender->freshest inbound sock */
